import { Component, ElementRef, OnDestroy, ViewChild, signal, Directive } from '@angular/core';
...
export abstract class PostEditorBase implements OnDestroy {
  ...
  protected objectUrls = new Map<string, string>();
  ...
  protected handleImagePaste(file: File): void {
    const objectUrl = URL.createObjectURL(file);
    const mediaId = this.generateMediaId();

    this.insertMedia(PostType.IMAGE, objectUrl, file.name, mediaId);
  ...
  handleVideoUpload(event: Event) {
    ...
    if (file && file.type.startsWith('video/')) {
      ...
      this.insertMedia(PostType.VIDEO, objectUrl, file.name, mediaId);
      this.objectUrls.set(mediaId, objectUrl);
  ...
  protected addFiles(files: File[]): void {
    ...
    for (const file of list) {
      const type: PostType = file.type.startsWith('video/') ? PostType.VIDEO : PostType.IMAGE;
      const objectUrl = URL.createObjectURL(file);
      const mediaId = this.generateMediaId();
  ...
  insertMedia(type: PostType, src: string, filename: string, mediaId: string) {
  ...
  protected updateMediaPositions() {
  ...
  protected insertHtmlAtCursor(html: string): void {
    const selection = window.getSelection();
  ...
  onContentChange(event: Event) {
    ...
    this.postForm.patchValue({ content: this.currentContent });
